...
(function(factory) {
    ...
    EvoCalendar = (function() {
        ...
        function EvoCalendar(element, settings) {
            ...
            _.initials = {
                ...
                dates: {
                    en: {
                        ...
                        daysShort: ["Sun", "Mon", "Tue", "Wed", "Thu", "Jum", "Sab"],
            ...
            _.$cal_days_labels = ['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'];
            ...
            _.$cal_current_date = new Date(_.options.initialDate);
            ...
            _.$active_date = _.formatDate(new Date(_.options.initialDate), _.options.format, 'en');
    ...
    EvoCalendar.prototype.init = function(creation) {
        ...
        if (!$(_.$calendar).hasClass('calendar-initialized')) {
            ...
            if(_.options.has_content){
                $(_.$calendar).addClass('evo-calendar calendar-initialized sidebar-hide');
            }else{
    ...
    EvoCalendar.prototype.buildCalendar = function(val, new_month, new_year) {
        ...
        function buildSidebarHTML() {
            sidebarHTML = '<div class="calendar-year">';
            sidebarHTML += '  <button class="icon-button" year-val="prev" title="Previous year">';
            sidebarHTML += '    <span class="chevron-arrow-left"></span>';
            sidebarHTML += '  </button>';
            sidebarHTML += '  &nbsp;<p>'+new_year+'&nbsp;</p>';
            sidebarHTML += '  <!--<button class="icon-button" year-val="next" title="Next year">';
            sidebarHTML += '    <span class="chevron-arrow-right"></span>';
            sidebarHTML += '   </button>-->';
            sidebarHTML += '</div>';
                ...
                for (var i = 0; i < _.options.calendarEvents.length; i++) {
                    if(_.$active_date === _.options.calendarEvents[i].date) {
                        ...
                        if(_.options.has_content) {
                            ...
                            eventHTML += '<div class="event-info">' + _.options.calendarEvents[i].name + '</div>';
                            eventHTML += '<div class="evo_calendar_content"></div>';
                            eventHTML += '</div>';    
                        }else{
                            eventHTML += '<div class="event-container">';
                            eventHTML += '<div class="event-icon"><div class="event-bullet-'+_.options.calendarEvents[i].type+'"></div></div>';
